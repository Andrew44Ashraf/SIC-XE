...
def main():
    ...
    for line in lines:
        ...
        if len(lmo) == 1:       # No label and no operands
            ...
            objectcode.append(makeInstruction(lmo[0],"#0",(curloc+assembledLength(lmo[0],operands))))
            ...
            objectcode.append(makeInstruction(mnemonic,operands,(nextLoc)))
            if mnemonic in ["START","END","BASE","NOBASE"]:
                print "\t%s\t%s\t%s\t%s"%(label,mnemonic,operands,instruction)
            else:
                print "%05X\t%s\t%s\t%s\t%s"%(curloc,label,mnemonic,operands,instruction)
            curloc=nextLoc
    for i in range (0,len(objectcode)-1):
        obcode = str(objectcode[i])
        outfile.write(obcode + " ")
main()
